...
async def receive_data(data: Data):
    try:
        ...
        # 받은 GPS 데이터 로그 출력
        print(
            f"[{timestamp}] Received GPS Data - Latitude: {data.latitude}, Longitude: {data.longitude}, Altitude: {data.altitude}, Heading: {data.heading}")
        ...
        if len(center_coordinates) > 0:
            ...
            # 검출된 이미지를 저장할 경로 생성
            detected_image_filename = os.path.join(DETECTED_DIR,
                                                   f"detected_{timestamp.replace(':', '').replace(' ', '_')}.jpg")
